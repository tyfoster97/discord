...
function kick(member, args) {
  member.kick(getReason(args)).catch(err => {
    console.log(err);
    return false;
  });
  return true;
...
module.exports.run = async (client, message, args) => {
  const author = message.guild.member(message.author); //if author has permission to kick users
      ...
      if (message.author.username != user.username) {
        ...
        if (member) {
          ...
          if (!member.hasPermission('KICK_MEMBERS') && kick(member, args)) {
            const msg = new MessageEmbed().setColor(color).setTitle('Member kicked').setDescription(`Successfully kicked ${user.tag}`).addField('Reason', getReason(args));
            message.reply(msg);
          } else {
            const msg = new MessageEmbed().setColor(color).setTitle('Unable to kick member').setDescription(`Could not kick ${user.tag}`);
