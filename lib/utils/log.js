...
async function errorLog(client, message, error) {
  const msg = new MessageEmbed().setColor(warn).setTitle('Error Log').addFields({
    name: 'User',
  ...
  client.channels.cache.get(id).send(msg); //client.channels.cache.get(id).send(error.stack.toString());
}
...
async function infoLog(client, info) {
  const msg = new MessageEmbed().setColor(color).setTitle('INFO').setDescription(info);
...
async function inform(message) {
  //build message embed
  const msg = new MessageEmbed().setColor(color).setTitle('Unexpected Error').setDescription('Something happened on our end\n' + 'We have logged the error and may contact you if we need more details.\n\n' + 'This message will delete after 60 seconds'); //send
